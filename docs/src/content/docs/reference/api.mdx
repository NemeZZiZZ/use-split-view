...
| Option | Type                   | Default | Description                                                                                         |
| ------ | ---------------------- | ------- | --------------------------------------------------------------------------------------------------- |
| `zoom` | `SplitViewZoomOptions` | —       | Extra config forwarded to the underlying `useZoomPinch` (bounds, inertia, keyboard, rotation, …). See [Advanced Zoom Options](/use-split-view/guides/advanced-zoom/). |
...
| `centerZoom`      | `(targetZoom: number, opts?: AnimationOptions) => void` | Zoom to target keeping container center fixed, optionally animated |
| `resetView`       | `(opts?: AnimationOptions) => void` | Reset to `{ x: 0, y: 0, zoom: 1 }`, optionally animated |
...
### Advanced zoom helpers

| Property   | Type               | Description                                                                                       |
| ---------- | ------------------ | ------------------------------------------------------------------------------------------------- |
| `zoomApi`  | `SplitViewZoomApi` | Imperative helpers from the underlying `useZoomPinch` instance: `zoomIn`, `zoomOut`, `zoomTo`, `panTo`, `panBy`, `fitToRect`, `fitToContent`, `zoomToElement`, `rotateTo`, `rotateBy`, `snapZoom`, `screenToContent`, `contentToScreen` |

`zoomApi` points at the **same** instance that drives the panes — use it for operations not exposed directly on the return object:
